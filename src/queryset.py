...
class QuerySet:
    ...
    def __init__(self, query_k=1, do_both_phases=True, save_results=True, vaqindex:VAQIndex=None):
        self.do_both_phases = do_both_phases # not using yet
        self.save_results = save_results # not using yet
        self.VAQ = vaqindex # Can access cset from here
        self.query_k = query_k

        self.full_query_fname = None
        self.full_res_fname = None # Output result set file -> set in init
        ...
        # Will extract from dataset vars file
        self.dim_means = None
    ...
    def _initialize(self):
        ...
        # Set up filenames
        self.full_query_fname = os.path.join(self.VAQ.TDS.DS.path, '') + self.VAQ.TDS.DS.fname + '_qry'
        self.full_res_fname = os.path.join(self.VAQ.TDS.DS.path, '') + self.VAQ.TDS.DS.fname + '.res'
        self.full_metrics_fname = os.path.join(self.VAQ.TDS.DS.path, '') + self.VAQ.TDS.DS.fname + '.met'

        # Get variables from VAQIndex
        self.boundary_vals = self.VAQ.boundary_vals
        self.cells = self.VAQ.cells
    ...
    def _transform_query_file(self):
        ...
        # Currently getting data_mean and transform_matrix from file. Alternative: get them by looking back through other classes?
    ...
    def _run_phase_one(self, query_idx):
        ...
        self.ANS = np.ones((self.query_k), dtype=np.float32)*np.inf
        self.UP = np.ones((self.query_k), dtype=np.float32)*np.inf
        self.V = np.ones((self.query_k), dtype=np.uint32)*np.inf

        # These are all (num_vectors, 1)
        self.L = np.zeros((self.VAQ.TDS.DS.num_vectors,1), dtype=np.float32)
        self.U = np.zeros((self.VAQ.TDS.DS.num_vectors,1), dtype=np.float32)
        self.S1 = np.zeros((self.VAQ.TDS.DS.num_vectors,1), dtype=np.float32)
        self.S2 = np.zeros((self.VAQ.TDS.DS.num_vectors,1), dtype=np.float32)

        # Set up VAQIndex generator (one block = one dimension)
        vaq_gene = self.VAQ.generate_vaq_block()
        
        num_vectors_per_block = self.VAQ.TDS.DS.num_vectors_per_block
        num_blocks = self.VAQ.TDS.DS.num_blocks
        ...
        for CSET in vaq_gene: # block = cset -> (num_vectors, 1)
            ...
            for i in range(num_blocks): 
                ...
                d1 = np.abs(qj_rep - self.boundary_vals[cset+1, block_count])
                d2 = np.abs(qj_rep - self.boundary_vals[cset, block_count]) 
        ...
        # Elim counting
        elim = 0
        for i in range(self.VAQ.TDS.DS.num_vectors):
            if self.L[i] <= self.UP[self.query_k - 1]:
                ...
                UP_appended_sorted = np.sort(np.append(self.UP, self.U[i])) # Diff variable names than MATLAB
                self.UP = UP_appended_sorted[0:self.query_k]
            else:
                elim += 1
    ...
    def _run_phase_two(self, query_idx):
        ...
        num_words_random_read = self.VAQ.TDS.DS.num_dimensions
        ...
        for i in range(self.VAQ.TDS.DS.num_vectors):
            ...
            if LL[i] > self.ANS[self.query_k -1]:
                break
            # Else -> viable option
            else:
                ...
                start_offset = J[i]*self.VAQ.TDS.DS.num_dimensions*self.VAQ.TDS.DS.word_size
                TSET = self.VAQ.TDS.tf_random_read(start_offset[0], num_words_random_read)
    ...
    def _load_dataset_vars(self):
        ...
        print("Loading dataset variables from ", self.VAQ.TDS.DS.path)

        # Just reading from .dsvars.npz currently; could work on trimming off the .npz if we want.
        with np.load(os.path.join(self.VAQ.TDS.DS.path, '') + self.VAQ.TDS.DS.fname + '.dsvars.npz') as data:
