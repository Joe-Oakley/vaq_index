...
class DataSet:
    ...
    def _initialise(self):
        ...
        assert (total_file_words / self.num_blocks) % (
                self.num_dimensions + 1) == 0, "Inconsistent number of blocks selected."
    ...
    def _calc_dim_means(self):
        ...
        for i in range(self.num_blocks):
            ...
            dim_sums = np.divide(np.sum(block, axis=0).reshape(1, self.num_dimensions), self.num_vectors_per_block)
            self.dim_means = np.add(self.dim_means, dim_sums)

        self._close_file()
        self.dim_means = np.divide(self.dim_means, self.num_blocks)
    ...
    def _calc_cov_matrix(self):
        ...
        self.cov_matrix = np.divide(self.cov_matrix, self.num_blocks)
